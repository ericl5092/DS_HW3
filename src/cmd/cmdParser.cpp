...
CmdParser::openDofile(const string& dof)
{
   ...
   {   
       cerr<<"Error: dofile stack overflow (1024)"<<endl;
       return false;
   }else
   {
...
CmdParser::parseCmd(string& option)
{
   ...
   string cmd;
   size_t end_pos = myStrGetTok(str,cmd);
   CmdExec* e = getCmd(cmd);
   if(!e)
   {
       cerr<<"Illegal command!! \"(string "<<str<<")"<<endl;
       return NULL;
   }
   option = (end_pos==string::npos)?"": str.substr(end_pos);
   return e;
}
...
//
// v2. [Before] partially matched (multiple matches)
...
//
// v3. [Before] partially matched (single match)
//    cmd> he$
//    [After Tab]
//    cmd> heLp $
//    -----------
...
CmdParser::listCmd(const string& str)
{
   ...
   size_t end_pos = myStrGetTok(str,token);
   ...
   {
       vector<map<const string,CmdExec*>::iterator> it_cmds;
       it_cmds.reserve(_cmdMap.size());
       for(it = _cmdMap.begin();it!=_cmdMap.end();it++)
       {
           string cmd = it->first + it->second->getOptCmd();
           if (cmd.size()<token.size()){mybeep();return;}
           cmd.resize(token.size());
           if(myStrNCmp(cmd,token,token.size())==0){it_cmds.push_back(it);}
       }
       if(it_cmds.size()==0)
       {mybeep();return;}else if(it_cmds.size()==1)
       {
           string cmd = it_cmds[0]->first+it_cmds[0]->second->getOptCmd();
           if (token.size()==cmd.size())
           {
               it_cmds[0]->second->help();
               reprintCmd();
           }else if(str.substr(end_pos).size()!=0){mybeep();return;}else
           {
               for(int i = token.size();i<cmd.size();i++)
               {
                   insertChar(cmd[i]);
               }
               insertChar(' ');
        ...
        {
           if(str.substr(end_pos).size()!=0){mybeep();return;}
           int count = 0;
           for(vector<map<const string, CmdExec*>::iterator>::iterator it_v = it_cmds.begin();it_v!=it_cmds.end();it++)
           {
               string cmd = (*it_v)->first +(*it_v)->second->getOptCmd();
               cout << setw(12) << left << cmd;
               if(count%5==4)
               {
                   cout<<endl;
...
CmdParser::getCmd(string cmd)
{
   ...
   while(size>0)
   {
       it = _cmdMap.find(mand_cmd);
       if(it!=_cmdMap.end()) break;
       mand_cmd.resize(--size);
   }
   if(size==0)
   {return e;}else
   {
       string opt_cmd = cmd.substr(size);
       const string& cmp_opt_str = (it->second)->getOptCmd();
       if(opt_cmd.size()<=(cmp_opt_str).size())
       {
           for(int i =0; i<opt_cmd.size();i++)
           {
               if(opt_cmd[i]!=toupper(cmp_opt_str[i])) return e;
           }
           e = it->second;
           return e;
       }
