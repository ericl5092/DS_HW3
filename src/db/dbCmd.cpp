...
initDbCmd()
{
   ...
   if(!(cmdMgr->regCmd("DBAPpend",4,new DBAppendCmd)&&
       cmdMgr->regCmd("DBAVerge",4,new DBAveCmd)&&
       ...
       {
           cerr<<"Registering \"init\"i command fails... exiting"<<endl;
           return false;
       }
   return true;
}
...
DBAppendCmd::exec(const string& option)
{
   ...
   if(options.size()==1)
   {
       cerr<<"Missing option after "<<options[0]<<"!!";   
   }
   if (doRow) {
               ...
               else
               {
                   if(options[i+1]=="-")
                   {
                       new_row->addData(INT_MAX);
                        ...
                        {cerr<<"\""<<options[i+1]<<"\" is not an integer!!";return CMD_EXEC_ERROR;}
               }
           }
       }
       dbtbl.addRow(*new_row);
   }
   else {
       if(options.size()==2&&options[1]=="-")
       {
       ...
       }else
       {
           ...
           {
               int num;
               if(!(myStr2Int(options[i+1],num))&&options[i+1]!="-")
               {cerr<<"\""<<options[i+1]<<"\" is not an integer!!";return CMD_EXEC_ERROR;}
           }
...
{  
   ...
   else if (myStrNCmp("-Column", options[0], 2) != 0)
   {
       ...
       }else if(myStr2Int(options[0],num))
       {
            ...
            if(myStr2Int(options[1],col_num))
            {
                int c,r;
                if(!checkRowIdx(options[0],r)||!checkColIdx(options[1],c)){return CMD_EXEC_ERROR;}
                dbtbl.printData(cout,dbtbl[num][col_num]);
                return CMD_EXEC_DONE;
            }else
            {
                cerr<<"\""<<options[0]<<"\"is not an integer!!";
                return CMD_EXEC_ERROR;
            }
       }else
       {cerr<<"\""<<options[0]<<" is not an integer!!";return CMD_EXEC_ERROR;}
